						...
						<c:choose>
							<c:when test="${fn:length(list) > 0 }">
								<table style="width: 100%; font-size: smaller;">
									<tr>
										<th>no</th>
										<th><select onchange="selectIp()" id="ip">
												<option>ip를 선택</option>
												<c:forEach items="${ipList }" var="i">
													<c:if test="${ip eq i }">
														<option value="${i }" selected="selected">${i }</option>
													</c:if>
													<c:if test="${ip ne i }" var="i">
														<option value="${i }">${i }</option>
													</c:if>
												</c:forEach>
										</select></th>
										<th>date</th>
										<th><select onchange="selectTarget()" id="target">
												<option>target을 선택</option>
